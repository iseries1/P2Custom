...
{
  int i;
  _em7180 = I2C_Init(clk, sda, I2C_STD);
...
short Em7180_readbarometer()
{
  int i;

  _readBytes(EM7180_ADDRESS, EM7180_Baro, 2, _buffer); 
  i = (_buffer[1] << 8) | _buffer[0];
  return i;
}

short Em7180_readtemperature()
{
  int i;

  _readBytes(EM7180_ADDRESS, EM7180_Temp, 2, _buffer);
  i = (_buffer[1] << 8) | _buffer[0];
  return i;
}
...
void Em7180_setPassThroughMode(void)
{
  ...
  i = _readByte(EM7180_ADDRESS, EM7180_AlgorithmControl);
  _writeByte(EM7180_ADDRESS, EM7180_AlgorithmControl, i | 0x01); // standby mode
  _writeByte(EM7180_ADDRESS, EM7180_PassThruControl, 0x01);
  if (_readByte(EM7180_ADDRESS, EM7180_PassThruStatus) & 0x01)
  {
...
void _writeByte(unsigned char address, unsigned char subAddress, unsigned char data)
{
  ...
  i = I2C_Out(_em7180, address, subAddress, 1, &data, 1);
}
...
uint8_t _readByte(unsigned char address, unsigned char subAddress)
{
  ...
  i = I2C_In(_em7180, address, subAddress, 1, &data, 1);
  return data;
}
